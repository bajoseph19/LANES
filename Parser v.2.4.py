...
def get_ingredients(url):  
    ...
    Ingredient = namedtuple("Ingredient", ["s","p","qu_m","l_ns","l_vb","l_adj","l_r"])
    Item = namedtuple("Item", ["s","qu_r","qu_m"])
    Line = namedtuple("Line", ["s","p","l_t","l_ns","l_vb","l_adj","l_r"])
    ...
    lookups_qu_m = [
            (r'(?P<qu_1>(?:\s)?\d+\.?\d{0,2}(?:\s)?)?(?P<or>\-)?(?P<qu_2>(?:\s)?\d+\.?\d{0,2})(?P<measure>(?:\s)fluid ounces?(?:\s|$)|(?:\s)fluid ozs?\.?(?:\s|$)|(?:\s)fl\.? ounces?(?:\s|$)|(?:\s)fl\.? ozs?\.?(?:\s|$)|(?:\s)pints?(?:\s|$)|(?:\s)pts?\.?(?:\s|$)|(?:\s)gallons?(?:\s|$)|(?:\s)gals?\.?(?:\s|$)|(?:\s)quarts?(?:\s|$)|(?:\s)qts?\.?(?:\s|$)|(?:\s)millilit[re]{2,2}s?(?:\s|$)|(?:\s)mls?\.?(?:\s|$)|(?:\s)lit[re]{2,2}s?(?:\s|$)|(?:\s)ls?\.?(?:\s|$)|(?:\s)teaspoons?(?:\s|$)|(?:\s)tsp.?(?:\s|$)|(?:\s)tablespoons?(?:\s|$)|(?:\s)tbl\.?(?:\s|$)|(?:\s)tbsp?.?(?:\s|$)|(?:\s)cups?(?:\s|$)|(?:\s)ounces?(?:\s|$)|(?:\s)ozs?\.?(?:\s|$)|(?:\s)pounds?(?:\s|$)|(?:\s)lbs?\.?(?:\s|$)|(?:\s)milligrams?(?:\s|$)|(?:\s)mgs?\.?(?:\s|$)|(?:\s)grams?(?:\s|$)|(?:\s)gs?.?(?:\s|$)|(?:\s)kilograms?(?:\s|$)|(?:\s)kgs?\.?(?:\s|$)|(?:\s)millimet[er]{2,2}s?(?:\s|$)|(?:\s)mms?\.?(?:\s|$)|(?:\s)centimet[er]{2,2}s?(?:\s|$)|(?:\s)cms?\.?(?:\s|$)|(?:\s)met[er]{2,2}s?(?:\s|$)|(?:\s)m\.?(?:\s|$)|(?:\s)inch(?:\s|$)|(?:\s)inches(?:\s|$))?(?P<unit>(?:\s)?cans?(?:\s|$)|(?:\s)?jars?(?:\s|$)|(?:\s)?packages?(?:\s|$)|(?:\s)?containers?(?:\s|$)|(?:\s)?cartons?(?:\s|$)|(?:\s)?box?(?:\s|$)|(?:\s)?packages?(?:\s|$)|(?:\s)?loafs?(?:\s|$))?','1')
            ]
    lookups_item = [
            (r'(?P<words>\w+(?:\s|$))','words')
            ]
    ...
    def get_qu_m(c):
        qu_m = Item(s=c,qu_r=c,qu_m='None')
        for pattern, value in lookups_cc:
            if re.search(pattern, c):
                cc = re.search(pattern, c)
                for pattern, num in lookups_qu_m:
                    if re.search(pattern,cc.group('b_'+value)) and re.search(pattern,cc.group('a_'+value)):
                        r = re.sub(value,'',c)
                        r = re.sub(pattern,'',r)
                        p = []
                        qu = ''
                        m_b = []
                        fa_b = re.findall(pattern,cc.group('b_'+value))
                        p.append([index for b in fa_b for index,x in enumerate(b) if x == ''])
                        for index,pat in enumerate(p):
                            print pat
                            if pat == []:
                                qu = fa_b[index][0]
                                m_b += [qu,(fa_b[index][2],fa_b[index][3],fa_b[index][4]),'or']
                                qu = fa_b[index][1]
                                m_b += [qu,(fa_b[index][2],fa_b[index][3],fa_b[index][4])]
                            elif pat == [1]:
                                qu = fa_b[index][0]
                                m_b += [qu,(fa_b[index][2],fa_b[index][3],fa_b[index][4])]
                            elif pat == [3]:
                                qu = fa_b[index][0]
                                m_b += [qu,(fa_b[index][2],'m',fa_b[index][4]),'or']
                                qu = fa_b[index][1]
                                m_b += [qu,(fa_b[index][2],'m',fa_b[index][4])]    
                            elif pat == [4]:
                                qu = fa_b[index][0]
                                m_b += [qu,(fa_b[index][2],fa_b[index][3],'item'),'or']
                                qu = fa_b[index][1]
                                m_b += [qu,(fa_b[index][2],fa_b[index][3],'item')]    
                            elif pat == [0,1]:
                                qu = '1'
                                m_b += [qu,(fa_b[index][2],fa_b[index][3],fa_b[index][4])]
                            elif pat == [0,1,3]:
                                qu_hold_1 = fa_b[index][2]
                                hold_1 = ('qu_m','m',fa_b[index][4])
                            elif pat == [0,1,4]:
                                qu_hold_2 = '1'
                                hold_2 = (fa_b[index][2],fa_b[index][3],'item')
                            elif pat == [0,1,3,4]:
                                qu = fa_b[index][2]
                                m_b += [qu,('qu_m','m','item')]     
                            if [0,1,3] in p and [0,1,4] in p:   
                                qu = qu_hold_1
                                hold_2[0][2] = hold_1[0][2]
                                m_b += [qu,hold_2]
                            elif [0,1,3] in p:
                                qu = qu_hold_1
                                m_b += [qu,hold_1]
                        ...
                        hold_2 = []
                        m_a = []
                        fa_a = re.findall(pattern,cc.group('a_'+value))
                        p.append([index for a in fa_a for index,x in enumerate(a) if x == ''])   
                        for index,pat in enumerate(p):
                            print pat
                            if pat == []:
                                qu = fa_a[index][0]
                                m_a += [qu,(fa_a[index][2],fa_a[index][3],fa_a[index][4]),'or']
                                qu = fa_a[index][1]
                                m_a += [qu,(fa_a[index][2],fa_a[index][3],fa_a[index][4])]
                            elif pat == [1]:
                                qu = fa_a[index][0]
                                m_a += [qu,(fa_a[index][2],fa_a[index][3],fa_a[index][4])]
                            elif pat == [3]:
                                qu = fa_a[index][0]
                                m_a += [qu,(fa_a[index][2],'m',fa_a[index][4]),'or']
                                qu = fa_b[index][1]
                                m_a += [qu,(fa_a[index][2],'m',fa_a[index][4])]    
                            elif pat == [4]:
                                qu = fa_a[index][0]
                                m_a += [qu,(fa_a[index][2],fa_a[index][3],'item'),'or']
                                qu = fa_b[index][1]
                                m_a += [qu,(fa_a[index][2],fa_a[index][3],'item')] 
                            elif pat == [0,1]:
                                qu = '1'
                                m_a += [qu,(fa_a[index][2],fa_a[index][3],fa_a[index][4])]
                            elif pat == [0,1,3]:
                                qu_hold_1 = fa_a[index][2]
                                hold_1 = ('qu_m','m',fa_a[index][4])
                            elif pat == [0,1,4]:
                                qu_hold_2 = '1'
                                hold_2 = (fa_a[index][2],fa_a[index][3],'item')
                            elif pat == [0,1,3,4]:
                                qu = fa_a[index][2]
                                m_a += [qu,('qu_m','m','item')]     
                            if [0,1,3] in p and [0,1,4] in p:   
        ...
        for pattern, num in lookups_qu_m:
            if re.search(pattern, c):
                ...
                hold_2 = []
                m_m = []
                fa_m = re.findall(pattern, c)
                p.append([index for ma in fa_m for index,x in enumerate(ma) if x == ''])
                for index,pat in enumerate(p):
                    print pat
                    if pat == []:
                        qu = fa_m[index][0]
                        m_m += [qu,(fa_m[index][2],fa_m[index][3],fa_m[index][4]),'or']
                        qu = fa_m[index][1]
                        m_m += [qu,(fa_m[index][2],fa_m[index][3],fa_m[index][4])]
                    elif pat == [1]:
                        qu = fa_m[index][0]
                        m_m += [qu,(fa_m[index][2],fa_m[index][3],fa_m[index][4])]
                    elif pat == [3]:
                        qu = fa_m[index][0]
                        m_m += [qu,(fa_m[index][2],'m',fa_m[index][4]),'or']
                        qu = fa_b[index][1]
                        m_m += [qu,(fa_m[index][2],'m',fa_m[index][4])]    
                    elif pat == [4]:
                        qu = fa_m[index][0]
                        m_m += [qu,(fa_m[index][2],fa_m[index][3],'item'),'or']
                        qu = fa_b[index][1]
                        m_m += [qu,(fa_m[index][2],fa_m[index][3],'item')] 
                    elif pat == [0,1]:
                        qu = '1'
                        m_m += [qu,(fa_m[index][2],fa_m[index][3],fa_m[index][4])]
                    elif pat == [0,1,3]:
                        qu_hold_1 = fa_m[index][2]
                        hold_1 = ('qu_m','m',fa_m[index][4])
                    elif pat == [0,1,4]:
                        qu_hold_2 = '1'
                        hold_2 = (fa_m[index][2],fa_m[index][3],'item')
                    elif pat == [0,1,3,4]:
                        qu = fa_m[index][2]
                        m_m += [qu,('qu_m','m','item')]     
                    if [0,1,3] in p and [0,1,4] in p:   
    ...
    def get_li(s,qu_m,l_p,l_t):
        li = qu_m
        l_t = nltk.pos_tag(qu_m.qu_r.strip().split())
        l_r = s
        l_ns = []
        l_vb = []
        l_adj = []
        for w,pos in l_t:
            if pos in ['NN','NNS','NNP','NNPS']:
                l_ns += [w]
                l_r = re.sub(w,'',l_r)
            elif pos in ['VB','VBD','VBN','VBG']:
                l_vb += [w]
                l_r = re.sub(w,'',l_r)
            elif pos in ['JJ']:
                l_adj += [w]
                l_r = re.sub(w,'',l_r)
        li = Line(s=s,p=l_p,l_t=l_t,l_ns=l_ns,l_vb=l_vb,l_adj=l_adj,l_r=l_r)   
        return li
    
    def get_ing(s,qu_m,li):
        ing = Ingredient(s=s,p=li.p,qu_m=qu_m.qu_m,l_ns=li.l_ns,l_vb=li.l_vb,l_adj=li.l_adj,l_r=li.l_r)
        return ing
    ...
    for index, line in enumerate(final_recipe): 
        codified = codify(line,final_pattern[index],final_tag[index])
        print codified
