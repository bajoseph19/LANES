...
def get_ingredients(url):  
    ...
    lookups_qu_m = [
            (r'(?P<qu_1>\d+\.?\d{0,2} ){1,2}(?P<measure>fluid ounces?(?:\s|$)|fluid ozs?\.?(?:\s|$)|fl\.? ounces?(?:\s|$)|fl\.? ozs?\.?(?:\s|$)|pints?(?:\s|$)|pts?\.?(?:\s|$)|gallons?(?:\s|$)|gals?\.?(?:\s|$)|quarts?(?:\s|$)|qts?\.?(?:\s|$)|millilit[re]{2,2}s?(?:\s|$)|mls?\.?(?:\s|$)|lit[re]{2,2}s?(?:\s|$)|ls?\.?(?:\s|$)|teaspoons?(?:\s|$)|tsp.?(?:\s|$)|tablespoons?(?:\s|$)|tbl\.?(?:\s|$)|tbsp?.?(?:\s|$)|cups?(?:\s|$)|ounces?(?:\s|$)|ozs?\.?(?:\s|$)|pounds?(?:\s|$)|lbs?\.?(?:\s|$)|milligrams?(?:\s|$)|mgs?\.?(?:\s|$)|grams?(?:\s|$)|gs?.?(?:\s|$)|kilograms?(?:\s|$)|kgs?\.?(?:\s|$)|millimet[er]{2,2}s?(?:\s|$)|mms?\.?(?:\s|$)|centimet[er]{2,2}s?(?:\s|$)|cms?\.?(?:\s|$)|met[er]{2,2}s?(?:\s|$)|m\.?(?:\s|$)|inch(?:\s|$)|inches(?:\s|$))?(?P<unit>cans?(?:\s|$)|jars?(?:\s|$)|packages?(?:\s|$)|containers?(?:\s|$)|cartons?(?:\s|$)|box?(?:\s|$)|packages?(?:\s|$)|loafs?(?:\s|$))?','1')
            ]
    lookups_item = {
            'st':r'(?P<unit_1>(?P<qu_1>CD ?){1,2}(?P<m_1>NNS? ?){0,1}){1,1}(?P<item>((JJ ?)*(VB(D|N|G|P)? ?)*)*(NNS? ?){1,3}){1,1}',
            'cc':r'(?P<cc>( CC ){1})',
            'ccd':r'(?P<cc>( CC CD){1})',
            's_s_i':r'(?P<unit_1>(?P<qu_1>CD ?){1,2}(?P<m_1>NNS? ?){0,1}){1,1}(?P<item_1>((JJ ?)*(VB(D|N|G|P)? ?)*)*(NNS? ?){1,3}){1,1} CC (?P<unit_2>(?P<qu_2>CD ?){1,2}(?P<m_2>(NNS? ?)){0,1}){1,1}(?P<item_2>((JJ ?)*(VB(D|N|G|P)? ?)*)*(NNS? ?){1,3}){1,1}',
            'u_s_uui':r'(?P<unit_1>(?P<qu_1>CD ?){1,2}(?P<m_1>NNS? ?){0,1}){1,1} CC (?P<unit_2>(?P<qu_2>CD ?){1,2}(?P<m_2>(NNS? ?)){0,1}){1,1}(?P<item_1>((JJ ?)*(VB(D|N|G|P)? ?)*)*(NNS? ?){1,3}){1,1}',
            'u_s_uiu':r'(?P<unit_1>(?P<qu_1>CD ?){1,2}(?P<m_1>NNS? ?){0,1}){1,1}(?P<item_1>((JJ ?)*(VB(D|N|G|P)? ?)*)*(NNS? ?){1,3}){1,1} CC (?P<unit_2>(?P<qu_2>CD ?){1,2}(?P<m_2>(NNS? ?)){0,1}){1,1}',
            'u_s_uii':r'(?P<unit_1>(?P<qu_1>CD ?){1,2}(?P<m_1>NNS? ?){0,1}){1,1}(?P<item_1>((JJ ?)*(VB(D|N|G|P)? ?)*)*(NNS? ?){1,3}){1,1} CC (?P<item_2>((JJ ?)*(VB(D|N|G|P)? ?)*)*(NNS? ?){1,3}){1,1}',
            'u_s_uiui':r'(?P<unit_1>(?P<qu_1>CD ?){1,2}(?P<m_1>NNS? ?){0,1}){1,1}(?P<item_1>((JJ ?)*(VB(D|N|G|P)? ?)*)*(NNS? ?){1,3}){0,1} CC (?P<unit_2>(?P<qu_2>CD ?){1,2}(?P<m_2>(NNS? ?)){0,1}){0,1}(?P<item_2>((JJ ?)*(VB(D|N|G|P)? ?)*)*(NNS? ?){1,3}){0,1}',
    ...
    def get_qu_m(s,l_p,l_t):
        ...
        for pattern, value in lookups_cc:
            ...
            cc = re.search(lookups_item['cc'], s)
            if value == 'or':
        ...
        if re.search(lookups_item['cc'], l_p):
            if re.search(lookups_item['ccd'], l_p):
                ...
                for key in keys:
                    if re.search(lookups_item[key], l_p):
                        for m in re.finditer(lookups_item[key], l_p):
                            if (len(m.group('unit_1').split()) == len(m.group('unit_2').split()) or key =='s_s_u'):
                                ...
                                m_m = [key,m.group('qu_1'),m.group('m_1'),m.group('unit_1'),m.group('qu_2'),m.group('m_2'),m.group('unit_2'),lookups_item[key],l_p,s,url,finalMap]
                                return m_m
                            else:
                                continue
                        else:
                    ...
                    m_m = ['','None','None','None','None','None','None','',l_p,s,url,finalMap]
                    return m_m
            else:
                if re.search(lookups_item['u_s_uii'], l_p):
                    for m in re.finditer(lookups_item['u_s_uii'], l_p):
                       if len(m.group('item_1').split()) == len(m.group('item_2').split()):
                           ...
                           m_m = ['u_s_uii',m.group('qu_1'),m.group('m_1'),m.group('unit_1'),'None','None','None',lookups_item['u_s_uii'],l_p,s,url,finalMap]
                           return m_m
                       else:
                           continue
                    else:
            ...
            if re.search(lookups_item['st'], l_p):
                ...
                m_m = ['st',m.group('qu_1'),m.group('m_1'),m.group('unit_1'),'None','None','None',lookups_item['st'],l_p,s,url,finalMap]
                return m_m       
            else:
                m_w = Writer(re_key='',qu_1='None',m_1='None',unit_1='None',qu_2='None',m_2='None',unit_2='None',lookups_item_key='',	l_p=l_p,	s=s,	url=url,	finalMap=finalMap)
                m_m = ['','None','None','None','None','None','None','',l_p,s,url,finalMap]
