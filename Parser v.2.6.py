...
def get_ingredients(url):  
    ...
    lookups_item = [
            (r'(?P<unit>(CD ){1,2}(NNS? ){0,2}){1,1}(?P<item>((JJ ?)*(VBD ?)*)*(NNS? ?){1,3}){1,1}(?P<cc>(CC )?){0,1}(?P<unit_2>(CD ){1,2}(NNS? ){0,2}){0,1}(?P<item_2>(JJ ?)*(NNS? ?){1,3}){0,1}','1'),
            ]
    ...
    def get_qu_m(l_p,l_t):
        ...
        for pattern, value in lookups_item:
            if re.search(pattern, l_p):
                m = re.search(pattern, l_p)
                print l_p
                if m.group('unit') != None:
                    print m.group('unit')
                if m.group('item') != None:
                    print m.group('item')
                if m.group('cc') != None:
                    print m.group('cc')
                if m.group('unit_2') != None:
                    print m.group('unit_2')
                if m.group('item_2') != None:
                    print m.group('item_2')
        return qu_m
